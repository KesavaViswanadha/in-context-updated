...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None, replace_attn_func=None):
        super().__init__()

        if replace_attn_func: 
            self._attn = replace_attn_func
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None, attn_func=None):
        ...
        # self.MAMBA = MambaBlock(config, layer_idx)
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config, attn_func=None):
        ...
        self.h = nn.ModuleList([GPT2Block(config, layer_idx=i, attn_func=attn_func) for i in range(config.num_hidden_layers)])
        ...
        if not self.want_pos_embeddings:
            self.want_pos_embeddings = True
